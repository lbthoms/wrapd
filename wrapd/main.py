...
# Function to create and add tracks to a new playlist
def create_and_add_to_playlist(top_tracks, time_range):
    if 'spotify_token' not in session:
        return redirect(url_for('spotify_login'))
    ...
    playlist_name = f"Recommended Playlist - {time_range.capitalize()}"
    playlist = sp.user_playlist_create(user=session['spotify_user_id'], name=playlist_name, public=False)
